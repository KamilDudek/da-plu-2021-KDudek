...
async def get_suppliers_products(supplier_id: PositiveInt,
                                 db: Session = Depends(get_db)):
...
# Task 5.3
@router.post("/suppliers", response_model=schemas.SupplierBase,
             status_code=201)
...
# Task 5.4
@router.put("/suppliers/{supplier_id}", response_model=schemas.SupplierUpdate,
            status_code=200)
...
async def delete_supplier(supplier_id: PositiveInt,
                          db: Session = Depends(get_db)):
    db_supplier = crud.get_supplier(db, supplier_id)
    if not db_supplier:
        raise HTTPException(status_code=404, detail="Supplier not found")
